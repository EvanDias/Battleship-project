...
void shipToOne(SHIP *sh) {
    ...
    if(sh -> kind == SOLO) {
        setCellBP(sh -> bp, 0, 0, ternaryValue);

    } else if(sh -> kind == SMALL_QUAD || sh -> kind == BIGGEST_QUAD) {

        for(int i=0; i < sh -> columns; i++) {
            for(int j=0; j < sh -> rows; j++) {
               setCellBP(sh -> bp, i, j, ternaryValue);
            }
        }

    } else if(sh -> kind == TRIAL)  {
        for(int i=0; i<3; i++)
            setCellBP(sh -> bp, 0, i, ternaryValue);

    } else if(sh -> kind == L_GUY) {
        for(int i=1; i < sh -> columns; i++)
            setCellBP(sh -> bp, 4, i, ternaryValue);

        for(int j=0; j < sh -> rows; j++)
            setCellBP(sh -> bp, j, 0, ternaryValue);
    }

    else if(sh -> kind == T_GUY) {
        for(int i = 0; i < sh -> columns; i++) setCellBP(sh -> bp, i,1,ternaryValue);
        for(int j = 0; j < sh -> rows; j++) setCellBP(sh -> bp, 0,j, ternaryValue);
    }

    else if(sh -> kind == S_GUY) {
        for(int i = 0; i < 3; i++) setCellBP(sh -> bp, 4,i,ternaryValue);
        for(int j = 0; j < 5; j++) setCellBP(sh -> bp, j, 2, ternaryValue);
        for(int k = 3; k <= 4; k++) setCellBP(sh -> bp, 0,k,ternaryValue);
    }
  ...
  if(sh -> kind != L_GUY && sh -> kind != T_GUY && sh -> kind != S_GUY) {
    ...
    for(int i=0; i < sh -> columns; i++) {
      for(int j=0; j < sh -> rows; j++) {
        ...
        setCellBP(sh -> bp, j + sizeVertical, i + sizeHorizontal, '1');
        translate = true;
      }
      ...
      //  printf("Can't this values to translate! \n");
        ...
        for(int i = 0; i < sh -> columns;i++)  {
          ...
          setCellBP(sh -> bp, sizeVertical, i + sizeHorizontal, '1');
          setCellBP(sh -> bp, j + sizeVertical, 1 + sizeHorizontal, '1' );
          translate = true;
      }
